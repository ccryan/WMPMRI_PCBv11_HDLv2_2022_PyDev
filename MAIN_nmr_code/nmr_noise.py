...
def analyze( nmrObj, vvarac, samp_freq, samples, min_freq, max_freq, continuous, en_fig ):
    ...
    while True:
        ...
        specPwr_avg = specPwr_avg + spectPwr
        ...
        #==Cheng==
        if (i == avg_meas):
            specPwr_avg =  specPwr_avg/avg_meas
            fft_range = [i for i, value in enumerate( spectx ) if ( 
                value >= min_freq and value <= max_freq )]  # limit fft display
            fig = plt.figure()
            ax = fig.add_subplot(1, 1, 1)
            line1, = ax.plot( spectx[fft_range], specPwr_avg[fft_range], 'b-', label = 'data', linewidth = 0.5 )
            #plt.plot( spectx[fft_range], spectPwr[fft_range], 'b-', label = 'data', linewidth = 0.5 )
            
            peak_index = np.argmax(specPwr_avg[fft_range])

            # Peak value
            peak_value = spectx[fft_range[peak_index]]
